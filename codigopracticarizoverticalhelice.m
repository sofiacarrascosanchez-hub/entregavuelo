...
P_eje = 1500000;    % 1500 kW
eta_p = 0.8;        % 80% eficiencia
T_prop = (P_eje * eta_p) / V; 
...
fprintf('=== EMPUJE Y POTENCIA: RIZO (Reactor vs Hélice) ===\n');
fprintf('  T_req máximo:     %.0f N',max(T_req) );
fprintf('  T_max Reactor:    15800 N\n');
fprintf('  T_max Hélice:     %.0f N\n', T_prop);
...
if T_max > 15800
    fraccion_r = 100 * mean(T_req > 15800);
    fprintf('\n  [!] DIAGNÓSTICO: Déficit en el %.0f%% de la maniobra.\n\n', fraccion_r);
end
...
hold on;
yline(T_prop/1000, 'r--', 'LineWidth', 1.5, 'DisplayName', ...
    ['T_{prop} = ' num2str(T_prop/1000,'%.1f') ' kN (TFE731)']);

% Zona de déficit (si la hay)
if any(T_req > T_prop)
    area_x = phi_deg(T_req > T_prop);
    area_y = T_req(T_req > T_prop)/1000;
    fill([area_x, fliplr(area_x)], [area_y, (T_prop/1000)*ones(size(area_y))], ...
        'r','FaceAlpha',0.25,'EdgeColor','none','DisplayName','Déficit de empuje avión con hélice');
end
